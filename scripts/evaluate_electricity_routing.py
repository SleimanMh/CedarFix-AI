#!/usr/bin/env python3
"""Evaluate electricity routing guardrails."""
...
EVAL = ROOT / "data" / "eval" / "electricity_routing_eval_v1.jsonl"
...
def entity_matches(actual: str, expected: str) -> bool:
    if expected == "MUN":
        return actual not in {
            ...
            "OGERO",
            "MPWT",
            "PRIVATE_GENERATOR_OR_LOCAL_REGULATOR",
            "PRIVATE_PROPERTY_OR_ELECTRICIAN",
        }
...
def main() -> int:
    ...
    print(f"Electricity routing eval: {len(cases) - len(failures)}/{len(cases)} passed")
    if failures:
        print("Failures:")
