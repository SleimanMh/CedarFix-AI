...
- `compiled/routing_knowledge_compiled_production.json` is the committed
  production packaging export and default validation/seed input.
- `compiled/routing_knowledge_docs.jsonl` is an optional compiler output when
...
The seed script reads `compiled/routing_knowledge_compiled_production.json` by
default. It can also load JSONL when `ROUTING_KNOWLEDGE_DOCS` points to a JSONL
